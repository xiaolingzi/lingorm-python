...
class Query(QueryAbstract):
    ...
    def begin(self):
        self.__transaction_key = Native(
            self.__database_info, self.__transaction_key).begin()
        self.__native = NativeQuery(
            self.__database_info, self.__transaction_key)

    def commit(self):
        Native(self.__database_info, self.__transaction_key).commit()

    def rollback(self):
        Native(self.__database_info, self.__transaction_key).rollback()
